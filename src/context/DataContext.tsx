...
export function SupabaseDataProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<DataState>(() => ({ ...data, mode: 'production', loading, error, refresh,
    completeTask: (id) => mutate((repo) => repo.completeTask(id)), retryTask: (id) => mutate((repo) => repo.retryTask(id)), startRenewal: (id) => mutate((repo) => repo.startRenewal(id)), addCertificate: (certificate) => mutate((repo) => repo.createCertificate(certificate)), addCustomer: (input) => mutate((repo) => repo.createCustomer(input)), addEnvironment: (input) => mutate((repo) => repo.createEnvironment(input)), addDeployment: (input) => mutate((repo) => repo.createDeployment(input)), addRunbook: (input) => mutate((repo) => repo.createRunbook(input)), deleteRecord: (table, id) => mutate((repo) => repo.deleteRecord(table, id)), markNotificationsRead: () => mutate((repo) => repo.markNotificationsRead()),
