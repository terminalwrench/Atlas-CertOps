...
interface AccountMenuProps { name: string; email: string; organizationName: string; role: Role; onOpenSettings(): void; onSignOut(): Promise<void> }

export function AccountMenu({ name, email, organizationName, role, onOpenSettings, onSignOut }: AccountMenuProps) {
  ...
  return <div className="account-menu" ref={root}><button className="account-trigger" type="button" aria-label="Account menu" aria-haspopup="menu" aria-expanded={open} aria-controls={menuId} onClick={() => setOpen((value) => !value)}><span className="avatar">{initials}</span><span className="account-trigger-copy"><strong>{name || email}</strong><span>{role} role</span></span><ChevronUp size={15} className={open ? '' : 'account-chevron-closed'} /></button>{open && <div className="account-popover" id={menuId} role="menu" aria-label="Account"><div className="account-identity"><UserRound size={16} /><div><strong>{name || 'Signed-in user'}</strong><span>{email}</span><small>{organizationName} · {role}</small></div></div><button role="menuitem" type="button" onClick={() => { setOpen(false); onOpenSettings() }}><Settings size={15} />Account &amp; organization</button><button role="menuitem" type="button" onClick={() => void signOut()} disabled={busy}><LogOut size={15} />{busy ? 'Signing out…' : 'Sign out'}</button>{error && <p role="alert">{error}</p>}</div>}</div>
}
