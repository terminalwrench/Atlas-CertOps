...
export function Layout() {
  ...
  return <div className="app-shell">
    <aside className={open ? 'sidebar sidebar-open' : 'sidebar'}>
      ...
      <div className="sidebar-footer">{isDemo && <div className="demo-cta"><strong>Ready for your own estate?</strong><Link to="/signup">Start free</Link></div>}<div className="security-note"><ShieldCheck size={16} /><div><strong>Metadata only</strong><span>Private keys are never stored.</span></div></div>{isDemo ? <div className="profile"><div className="avatar">AM</div><div><strong>Alex Morgan</strong><span>Demo organization owner</span></div></div> : auth.user && auth.membership && <AccountMenu name={String(displayName ?? '')} email={auth.user.email ?? ''} role={auth.membership.role} onSignOut={async () => { await auth.signOut(); navigate('/login', { replace: true }) }} />}</div>
    </aside>
