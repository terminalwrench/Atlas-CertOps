...
describe('AccountMenu', () => {
  it('exposes identity and role through an accessible menu', () => {
    render(<AccountMenu name="Maya Chen" email="maya@example.com" role="admin" onSignOut={vi.fn()} />)
    ...
    expect(screen.getByText('admin · Production organization')).toBeInTheDocument()
  })
  ...
  it('awaits session sign-out and invokes the authenticated-shell redirect', async () => {
    ...
    render(<AccountMenu name="Maya Chen" email="maya@example.com" role="operator" onSignOut={signOutAndRedirect} />)
