...
export function DeploymentDetail() {
  ...
  return <>
    ...
    <div className="detail-grid">
      ...
      <section className="panel span-2"><div className="panel-heading"><div><h2>Operational state</h2><p>Current deployment, renewal, and validation proof</p></div>{currentWorkflow ? <Link to={`${path('/renewals')}#${currentWorkflow.id}`}>Open renewal operations</Link> : <StatusBadge>{deployment.status}</StatusBadge>}</div><div className="operational-facts"><div><span>Operational health</span><strong>{latestValidation ? (latestValidation.success ? 'Verified' : 'Validation Failed') : deployment.status}</strong></div><div><span>Last deployment</span><strong>{deployment.lastDeployment ? formatDate(deployment.lastDeployment) : 'Never'}</strong></div><div><span>Last validation</span><strong>{deployment.lastValidation ? formatDate(deployment.lastValidation) : 'Never'}</strong></div><div><span>Current renewal</span><strong>{currentWorkflow?.status ?? 'No active renewal'}</strong></div></div>{currentWorkflow && <WorkflowTimeline workflow={currentWorkflow} tasks={tasks} compact />}</section>
      <section className="panel span-2"><div className="panel-heading"><div><h2>Validation history</h2><p>Expected versus observed certificate evidence</p></div><StatusBadge>{latestValidation ? (latestValidation.success ? 'Verified' : 'Validation Failed') : 'Not validated'}</StatusBadge></div>{validations.length ? validations.map((validation) => <div className={`validation-row ${validation.success ? '' : 'validation-failed'}`} key={validation.id}><div className={validation.success ? 'validation-icon success' : 'validation-icon failure'}><ShieldCheck size={17} /></div><div className="grow"><strong>{validation.type} · {validation.success ? 'Verified' : 'Validation Failed'}</strong><span>Expected: {validation.expected}</span><small>Observed: {validation.actual}</small></div><time>{formatDate(validation.checkedAt)}</time></div>) : <p className="muted">No validation evidence has been recorded for this target.</p>}</section>
      <section className="panel"><div className="panel-heading"><div><h2>Deployment history</h2><p>Recorded execution evidence</p></div></div>{tasks.filter((task) => task.type !== 'Validation').length ? tasks.filter((task) => task.type !== 'Validation').map((task) => <div className="evidence-row" key={task.id}><div><strong>{task.title}</strong><span>{task.method} · {task.owner}</span></div><div><StatusBadge>{task.status}</StatusBadge><small>{task.completedAt ? formatDate(task.completedAt) : `Due ${formatDate(task.dueDate)}`}</small></div></div>) : <p className="muted">No workflow deployment history has been recorded.</p>}</section>
      ...
      <section className="panel span-2"><div className="panel-heading"><div><h2>Relationships</h2><p>Operational context for this deployment target</p></div></div><div className="relationship-strip"><Link to={customer ? path(`/customers/${customer.id}`) : path('/customers')}><span>Customer</span><strong>{customer?.name ?? 'Unassigned'}</strong></Link><div><span>Environment</span><strong>{deployment.environment}</strong></div><Link to={certificate ? path(`/certificates/${certificate.id}`) : path('/certificates')}><span>Certificate</span><strong>{certificate?.commonName ?? 'Unassigned'}</strong></Link><div><span>Owner</span><strong>{deployment.owner}</strong></div><Link to={path('/runbooks')}><span>Runbook</span><strong>{runbook?.title ?? 'Not associated'}</strong></Link><Link to={path('/activity')}><span>Evidence</span><strong>{validations.length} validations · {events.length} events</strong></Link></div></section>
